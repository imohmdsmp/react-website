...
const Authors02 = () => {
  ...
  const [listFlws, setListFlws] = useState([]);
  const [listFlwing, setListFlwing] = useState([]);
  useEffect(() => {
    if (!pathUser[2]) {
      if (profile.username) {
        axiosInstance
          .get(`/v1/post?username=${profile.username}`)
          .then((response) => {
            updatePost(response.data.data);
            setDataPost(response.data.data);
          });
        axiosInstance
          .get(`/v1/nft/collection?username=${profile.username}`)
          .then((response) => {
    ...
    if (profile.followers) {
      console.log(profile);
      setListFlws(profile.followers);
      setListFlwing(profile.following);
    }
  ...
  if (profile.social_link == null && profile.social_link == undefined) {
    ...
    instagramProfile =
      profile.social_link.instagram === "" ? (
        ...
        <li>
          <a
            href={`https://www.instagram.com/${profile.social_link.instagram}`}
            target="_blank"
          >
            <i className="fab  fa-instagram"></i>
          </a>
    ...
    twitterProfile =
      profile.social_link.twitter === "" ? (
        ...
        <li>
          <a
            href={`https://twitter.com/${profile.social_link.twitter}`}
            target="_blank"
          >
            <i className="fab fa-twitter"></i>
          </a>
    ...
    facebookProfile =
      profile.social_link.facebook === "" ? (
        ...
        <li>
          <a
            href={`https://www.facebook.com/${profile.social_link.facebook}`}
            target="_blank"
          >
            <i className="fab fa-facebook"></i>
          </a>
                ...
                <div className="d-flex flex-column flex-md-row justify-content-between">
                  <div className="infor-profile mt-5 pt-5">
                    ...
                    <div className="mt-4 d-flex flex-column flex-md-row text-center px-sm-5 mx-sm-5 px-md-0 mx-md-0 align-items-md-baseline">
                      <h5
                        className="sc-button mt-3 mr-0 mr-md-3"
                        onClick={() => {
                          connectToNFT();
                        }}
                      >
                    ...
                    <div className="btn-profile d-flex flex-column text-center">
                      ...
                      {pathUser[1] === "profile" ? (
                        <a
                          ...
                          target="_blank"
                          className="sc-button style-1 chat mt-3"
                        >
                          Chat
                        </a>
                      ) : (
                    ...
                    <TabPanel>
                      {dataPost.length === 0 ? (
                        <div className="wrapper-alert-nothing">
                          <p>Nothing To Show</p>
                          <Link to="/create-item" className="sc-button fs-2">
                            Create Post
                          </Link>
                        </div>
                      ) : (
                        dataPost.slice(0, visible).map((data, index) => (
                          <div
                            key={index}
                          ...
                          >
                            <div className="sc-card-product explode ">
                              ...
                              <div className="d-flex justify-content-center align-items-center">
                                <button
                                  className="sc-button mt-3 "
                                  onClick={() => PostToNFT(data.post_id)}
                                >
                                  Convert to NFT
                                </button>
                              </div>
